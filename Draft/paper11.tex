...
then builds structure on top of it. Each geometry $B_i$ is  determined by a
...
will have triangulated facets.  Such $B_i$ do not support NHC; see~\cite{Halverson:2016tve}.
...
\end{tikzpicture}
\end{center}
with the vertices and their heights labeled.
...
edge or face trees with $h \leq N$ grows rapidly with $N$, as in
...
these are the dominant ensembles.
...
Similar results hold for $\sdtc$. Let $A_1$ be the assumption that any simplex in an FRST 
...
of this physical property is $P(P_3)\geq P(A_3) \simeq .999995$. This non-trivial minimal gauge structure is universal in our large ensembles given by $\sdoc$ and $\sdtc$.
...
that is not in the largest facet.  Similar results and probabilities also hold
...
The gauge group on leaves above these $E_8$ factors on the ground is fixed entirely by their height. Such large gauge sectors motivate dark glueballs; see \cite{Halverson:2016nfq,*Soni:2016yes,*daRocha:2017cxu,*Acharya:2017szw,*Soni:2017nlm}.
...
this and related points.}. Here ``less than'' means that at least one of the multiplicities or orders is strictly less than the given multiplicity or order. We now translate this multiplicity of vanishing (MOV) condition to constraints on the height of the tree.
\begin{prop}
