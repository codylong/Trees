...
vs = pickle.load(open("alledgevertices.pickle",'rb'))
...
for face in p.faces(1):
    ...
    for i in range(len(faceip)):
        fip = faceip
        minlength, minind = 10^6, -1
        for j in range(i+1,len(faceip)):
            thismin = (fip[i][0]-fip[j][0])^2+(fip[i][1]-fip[j][1])^2+(fip[i][2]-fip[j][2])^2
            if thismin < minlength:
                minind, minlength = j, thismin
        possfacecones.append([faceip[i],faceip[minind]])
    possconesonf.append(possfacecones)
possconesone = possconesonf # we're really on edges here

for econes in possconesone:
    for c in econes:
        for i in range(len(vs)):
            if len(vs[i]) > 10: # max is 13, there are 4 12's, 8 11's
                nverts = pts[0:]
                for v in vs[i]:
                    newv = pl(ti(v[0],c[0]),ti(v[1],c[1]))
                    nverts.append(newv)
