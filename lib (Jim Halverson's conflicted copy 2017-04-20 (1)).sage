...
def singleBlowupConnectingFaceTrees(t1,t2): # False if not, 1 if t2 is blow of t1, -1 if t1 is blow of t2
...
def singleBlowupConnectingEdgeTrees(t1,t2): # False if not, 1 if t2 is blow of t1, -1 if t1 is blow of t2
    vs1, vs2 = t1, t2
    if abs(len(vs1)-len(vs2)) != 1: return False
    if len(vs1) > len(vs2): # make sure t1 is smaller tree for convenience
        b1,b2,c1,c2 = t1, t2, vs1, vs2
