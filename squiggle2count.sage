...
def genConfigs(config,oldsquigglecones,edgesfromlastblowup):
    ...
    squigglesmodsym = newsquiggles #sortedSquigglesModSymmetry(newsquiggles)

    #if symConfigInSet(config,genSymConfigs(c3)):
    #    print 'in here'
    #    print squigglesmodsym
    #    print len(newsquiggles)
    #    print len(newsquiggles), len(squigglesmodsym)
    #    for k in newsquiggles: 
    #        if k not in squigglesmodsym: print k
    #    print config
    #    ans1.extend(newsquiggles)
    #    ans2.extend(squigglesmodsym)
    #if configsEqual(config,c4): print 'strange'
    global thecount
    ...
    if thecount%100 == 0: print thecount
