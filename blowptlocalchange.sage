...
for p in polys:
    ...
    for facet in p.faces(2):
        ...
        oov = 1
        for i in range(len(interiorsindices)):
            for j in range(i+1,len(interiorsindices)):
                for l in range(j+1,len(interiorsindices)):
                    for k in range(len(mat)):
                        fks = sectionsipasssecs(k, fs)
                        gks = sectionsipasssecs(k, gs)
                        singfibk = True
                        for f in fks[0]:
                            if f[interiorsindices[i]]+f[interiorsindices[j]]+f[interiorsindices[l]]-8>=0:
                                singfibk = False
                                break
                        for g in gks[0]:
                            if g[interiorsindices[i]]+g[interiorsindices[j]]+g[interiorsindices[l]]-12>=0:
                                singfibk = False
                                break
                        if k not in integralpointsindices and singfibk: 
                            print "Sing outside facet", interiorsindices[i], interiorsindices[j], fks[0]
