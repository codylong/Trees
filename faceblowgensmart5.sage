...
def symConfigInSetW111(conf,confset):
    w111found = False
    for conf2 in confset:
        if numConesW111(conf)==numConesW111(conf2):
            w111found = True
            break
    if not w111found: return False
...
def configInSetW111(conf,confset):
    for conf2 in confset:
        if numConesW111(conf)==numConesW111(conf2) and configsEqual(conf,conf2): return True
    return False

def numConesW111(conf):
    num = 0
    for cone in conf:
        if (1,1,1) in cone: num += 1
    #print num
    return num
...
def decideHowToAdd(newconf):
    if satisfiesBenBound(newconf): # don't do anything else if not a good fit
        ...
        if len(configs[nConesM1]) == 0: # first time this number of cones
            ...
            configs[nConesM1].append([symverts,[newconf]])
            genConfigs(newconf)
        else:
            vertMatch = -1
            ...
            if vertMatch == -1: # first config or sym config with these verts
                ...
                if not symConfigInSetW111(newconf,configs[nConesM1][vertMatch][1]): # if sym conf not already this set
                    configs[nConesM1][vertMatch][1].append(newconf)
                    genConfigs(newconf)
...
# N=5 result: 4231, finished in 21 minute
