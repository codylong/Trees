...
def strat(cur):
	return floor(log(cur,2)) # just a hack to spread integers

def num():
	return sum([len(configs[i]) for i in range(len(configs))])

configs = [[] for i in range(500)]
...
def recurse(cur):
	...
	if len(configs[nc]) == 0:
		configs[nc] = [cur]
		blow = True
	elif configs[nc][len(configs[nc])-1] < cur:
		configs[nc].append(cur)
		blow = True
	else:
		for i in range(len(configs[nc])):
			if configs[nc][i] > cur:
				configs[nc].insert(i,cur)
				blow = True
				break
			elif configs[nc][i] == cur:
				break

	if blow:
		if n%1000==0: print n, str(datetime.datetime.now())
		for bl in bf:
