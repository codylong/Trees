...
vs = pickle.load(open("allvertices.pickle",'rb'))
...
size=21
bigtrees = [tree for tree in vs[0] if len(tree) > size]
for t0 in bigtrees:
    for t1 in bigtrees:
        for t2 in bigtrees:
            for t3 in bigtrees:
                for c0 in possconesonf[0]:
                    for c1 in possconesonf[1]:
                        for c2 in possconesonf[2]:
                            for c3 in possconesonf[3]:
                                nverts = pts[0:]
                                for v in t0:
                                    newv = pl(pl(ti(v[0],c0[0]),ti(v[1],c0[1])),ti(v[2],c0[2]))
                                    nverts.append(newv)
                                for v in t1:
                                    newv = pl(pl(ti(v[0],c1[0]),ti(v[1],c1[1])),ti(v[2],c1[2]))
                                    nverts.append(newv)
                                for v in t2:
                                    newv = pl(pl(ti(v[0],c2[0]),ti(v[1],c2[1])),ti(v[2],c2[2]))
                                    nverts.append(newv)
                                for v in t3:
                                    newv = pl(pl(ti(v[0],c3[0]),ti(v[1],c3[1])),ti(v[2],c3[2]))
                                    nverts.append(newv)
                                mk, m4k, m6k = [1 for v in nverts], [4 for v in nverts], [6 for v in nverts]
                                fsecs = sections(m4k,nverts)
                                gsecs = sections(m6k,nverts)
                                print (len(fsecs),len(gsecs))
...
#                         if len(vs[0][i])>size and len(vs[0][j])>size and len(vs[0][k])>size and len(vs[0][i])>size and 
...
#                 nverts = pts[0:]
#                 for v in vs[0][i]:
#                     newv = pl(pl(ti(v[0],c[0]),ti(v[1],c[1])),ti(v[2],c[2]))
#                     nverts.append(newv)
#                 mk, m4k, m6k = [1 for v in nverts], [4 for v in nverts], [6 for v in nverts]
#                 fsecs = sections(m4k,nverts)
#                 gsecs = sections(m6k,nverts)
