...
def inttoconfig(cur):
	return [cones[i] for i in range(len(cones)) if bini(cur,i)]

def conesintersect(c1,c2):
	if len([v for v in c1 if v in c2]) == 2: return True
	return False
...
for i in range(len(cones)):
	...
	for j in range(len(cones)):
		if i != j and conesintersect(cones[i],cones[j]):
			cur.append(2^j)
	adjcones.append(cur)
...
for bl in be:
	...
	facebls=[]
	for c in adjcones[log(bl[2],2)]:
		if c not in facebls: facebls.append(c)
	for c in adjcones[log(bl[3],2)]:
		if c not in facebls: facebls.append(c)
	for c in adjcones[log(bl[4],2)]:
		if c not in facebls: facebls.append(c)
	for c in adjcones[log(bl[5],2)]:
		if c not in facebls: facebls.append(c)
	

	for c in facebls:
		if bffind(c) != -1: cur.append(bffind(c))
...
def possbl(cur,bl):
	if len(bl)==6:
		return bini(cur,log(bl[0],2)) and bini(cur,log(bl[1],2))
	elif len(bl)==4:
		return bini(cur,log(bl[0],2))
